...
namespace VRSketch
{
    ...
    public struct SessionData
    {
        ...
        public SessionData(SketchSystem sketchSystem, InteractionMode mode, List<SystemState> systemStates, List<SerializableStroke> strokes, List<SerializablePatch> patches)
        {
    ...
    {
        public static void SaveData(SessionData sessionData, string fileName = "", string TerrainPath = "")
        {
            string path = Path.Combine(Application.dataPath, ExportController.GetExportFolderName());
            if (fileName.Length > 0)
                path = Path.Combine(path, TerrainPath);
            ...
            if (fileName.Length == 0)
                name = timestamp + "_" + interactionModeID + "_" + systemID + ".json";
        ...
        public static SessionData readJson(string fileName, SessionData sessionData)
        {
            string jsonContent = File.ReadAllText(fileName);
            sessionData = JsonConvert.DeserializeObject<SessionData>(jsonContent, new JsonSerializerSettings
            {
                Culture = new System.Globalization.CultureInfo("en-US")
            });
            return sessionData;
