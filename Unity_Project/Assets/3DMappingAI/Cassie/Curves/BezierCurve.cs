...
namespace Curve
{
    ...
    public class CubicBezier
    {
        ...
        /// </summary>
        /// <param name="t"></param>
        /// <returns>This line calculates the position on the curve by evaluating the cubic Bezier equation at the parameter t. </returns>
    ...
    public class BezierCurve : Curve
    {
        ...
        /// <summary>
        /// An anchor point, in the context of Bezier curves, is typically a start or end point of the curve. 
        /// </summary>
        ...
        /// </summary>
        /// <param name="t"></param>
        /// <returns>the segments index and the t within that segment</returns>
        private (int, float) ConvertPolyBezierParameter(float t)
        {
            int nSplits = beziers.Length; // total number of cubic Bezier curves that make up the poly-Bezier curve
            // Prevent index out of range on last point (t = 1)
            if (Mathf.Approximately(t, 1f))
                return (nSplits - 1, 1f);
            float t2 = t; 
            int splitIdx = Mathf.FloorToInt(t2 * nSplits);
            float u = t2 * nSplits - splitIdx; //U tells you where within this segment the global parameter t points to.
        ...
        /// <summary>
        /// computes the corresponding global parameter t for the entire poly-Bezier curve.
        /// </summary>
        ...
        private int SplitAt(int bezierIdx, float u)
        {
            ...
            //    return bezierIdx;
            //}
            List<Vector3> points = this.beziers[bezierIdx].GetPoints(); //  obtaining the control points of the Bezier curve
            // store the control points of the two new Bezier curves that will result from the split.
        ...
        {
            beziers = beziers.Take(oldBezier).//selecting all the Bezier curves before the one to be replaced
                Concat(newBeziers).
                Concat(beziers.Skip(oldBezier + 1)). //selecting all the elements after the replaced curve.
                ToArray();
        }
