...
namespace Curve {
    ...
    public abstract class Curve {
        ...
        // Method to get the cumulative lengths of segments along a curve. 
        // If no number of divisions is specified, a default is used.
        float[] GetLengths(int divisions = -1)
        {
            // If the divisions parameter is negative, set a default value of 200.
            // This means the curve will be divided into 200 segments by default.
            if (divisions < 0)
            {
                divisions = 200;
            }

            // Check if cached arc lengths exist and match the requested divisions,
            // and if the curve does not need updating.
            // This prevents recalculating the lengths if they have already been computed
            // for the same number of divisions and the curve has not changed.
            if (this.cacheArcLengths != null &&
                    (this.cacheArcLengths.Length == divisions + 1) &&
                    !this.needsUpdate)
            {
                return this.cacheArcLengths; // Return the cached lengths if conditions are met.
            }

            // If reaching this point, set needsUpdate to false as we're about to update the cache.
            this.needsUpdate = false;

            // Initialize an array to cache the new lengths, with one extra for the start point.
            var cache = new float[divisions + 1];
            Vector3 current, last = this.GetPoint(0f); // Initialize 'last' to the first point on the curve.

            cache[0] = 0f; // The first point has 0 length since it's the start.

            // Sum variable to accumulate the lengths of each segment.
            float sum = 0f;
            // Loop through each division, calculate and cache the lengths.
            for (int p = 1; p <= divisions; p++)
            {
                current = this.GetPoint(1f * p / divisions); // Get the point at the current division.
                sum += Vector3.Distance(current, last); // Add the distance from the last point.
                cache[p] = sum; // Cache the cumulative length up to this point.
                last = current; // Update 'last' to the current point for the next iteration.
            }

            // Return the array of cached lengths.
            return cache;
        }
        /// <summary>
        /// 
        ...
        /// <param name="N0">Initial Normal: If an initial normal vector N0 is given, it is used as the initial normal vector. Otherwise, an initial normal vector is selected perpendicular to the first tangent vector.</param>
