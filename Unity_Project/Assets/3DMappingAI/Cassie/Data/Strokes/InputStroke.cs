...
public class InputStroke : Stroke
{
    ...
    public bool ShouldUpdate(Vector3 candidateSamplePos, float inputSamplingDistance, float maxSamplingDistance)
    {
        if (Samples.Count == 0)
            return true;

        float distance = GetDistanceToSample(Samples.Count - 1, candidateSamplePos);
        if (distance > inputSamplingDistance && distance < maxSamplingDistance)
            return true;
        return false;
