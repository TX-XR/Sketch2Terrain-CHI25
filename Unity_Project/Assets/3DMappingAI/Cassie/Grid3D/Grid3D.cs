...
public class Grid3D : MonoBehaviour
{
    ...
    bool isCalibrationTypeChanged = false;
    private void OnEnable()
    {
        ...
        Sketch2TerrainEventManager.StartListening(Sketch2TerrainEventManager.SurfaceCalibrationReset, () => { isCalibrationTypeChanged = false; });
    }
    ...
    private void OnDisable()
    {
        ...
        Sketch2TerrainEventManager.StopListening(Sketch2TerrainEventManager.SurfaceCalibrationReset, () => { isCalibrationTypeChanged = false; });
    }


    void SurfaceCalibrationCompleted()
    {
        isCalibrationTypeChanged = true;
        ...
        var CalibrationManagementPos = FindObjectOfType<CalibrationManagement>().transform.position;
        //this.transform.position = LayerManagerPos;
        //Center = LayerManagerPos;
        Center = new Vector3(CalibrationManagementPos.x, CalibrationManagementPos.y, CalibrationManagementPos.z);
        ...
        //Debug.Log("gridAnchorPos" + new Vector3(gridMaxDim * 0.5f, gridMaxDim * 0.5f, gridMaxDim * 0.5f));
        //gridAnchorPos = Center;
    ...
    public void OnCanvasMove(bool flag = true)
    {
        if (!flag)
            return;
        // Update transform
    ...
    public void OnTransformStart(bool flag = true)
    {
        if (!flag)
            return;
    ...
    public void OnTransformEnd(bool flag = true)
    {
        if (!flag)
            return;
        material.SetFloat(_BaseOpacityIdx, BaseOpacity);
